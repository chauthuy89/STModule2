            ...
            try
            {
                ...
                if (profile != null)
                {
                    ...
                    foreach (var user in userList)
                    {
                        ...
                        var newTalent = new TalentSnapshotViewModel();
                        newTalent.Id = user.Id;
                        newTalent.Name = user.FirstName + " " + user.LastName;
                        newTalent.PhotoId = user.ProfilePhotoUrl;
                        newTalent.VideoUrl = user.VideoName;
                        newTalent.CVUrl = user.CvName;
                        newTalent.Summary = user.Summary;
                        if (user.Experience.Count > 0)
                        {
                            newTalent.CurrentEmployment = user.Experience[0].Company;
                            newTalent.Level = user.Experience[0].Position;
                        }
                        newTalent.Visa = user.VisaStatus;
                        newTalent.Skills = skills;

                        result.Add(newTalent);
                    }
                    return result;
                }
                return null;
